...
public class RotateToTarget : Node
{
    ...
    public override NodeStatus Excute()
    {
        ...
        owner.transform.rotation = Quaternion.RotateTowards(owner.transform.rotation, Quaternion.LookRotation(directionToTarget), owner.speedRotateToTarget *Time.deltaTime);
        float cosBodyTarget = Vector3.Dot(directionToTarget, owner.transform.forward);
        if (cosBodyTarget >= owner.cosMinSeeTarget)
        {
